...
/**
 * Client-side filter bar that writes state into the URL. The server page
 * reads it back from searchParams on each navigation — no client-side
 * data fetching, no React Query needed, bookmarkable URLs out of the box.
 */
export function LeadsFilterBar({ initial }: { initial: FilterState }) {
  ...
  return (
    <form
      className="grid grid-cols-1 gap-2 rounded-2xl border border-[var(--ds-border)] bg-[var(--ds-surface)] p-3 sm:grid-cols-6"
      onSubmit={(e) => {
        e.preventDefault();
        apply();
      }}
    >
      ...
      <select
        value={state.hasApplication}
        onChange={(e) => setState({ ...state, hasApplication: e.target.value })}
        className={INPUT_CLS}
      >
      ...
      <div className="flex items-center gap-2 sm:col-span-1">
        <button
          type="submit"
          className="rounded-lg bg-[var(--ds-primary)] px-3 py-1.5 text-sm font-semibold text-white"
        >
          Apply
        </button>
        <button
